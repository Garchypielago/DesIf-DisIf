package com.example.t10ej02.listeners;
...
public class ListenersController {
    ...
    private void initialize() {
    	...
    	// Cualquier nueva selecci�n del usuario se registra en el texto de debajo
    	...
    	// Cuando se detecta un nuevo �ndice se muestran todos aquellos ya seleccionados
    	// Para ObservableList, el listener de tipo change solo acepta un par�metro que incluye �mplitamente los valores modificados
    	...
    	// Esta ser�a la alternativa para InvalidationListener 
    	...
    	// Prueba de listener al a�adir un nuevo elemento
    	listNumeros.getItems().addListener((Observable observable) -> {
    		System.out.println("Invalidation al a�adir -> " + observable);
    	});
    	
    	listNumeros.getItems().add(10);
    	
    	// Aparte de los listeners y bind, vamos a probar a acceder din�micamente a un control
    	// Desde cualquier contenedor, con lookup podemos recuperar controles de la p�gina
    	// Adem�s, vamos a probar a seleccionar directamente desde un ToggleGroup del RadioButton
