	...
	public void start(Stage primaryStage) {
		try {
			...
			/*Diferencia entre eventos de teclado para validar con KeyPressed y KeyTyped
			  ...
			  Lo que sí que hay que tener cuidado es que si consumimos el evento KEY_PRESSED la tecla
			  que estamos impidiendo mostrar en realidad se visualiza porque KEY_TYPED se ejecuta después */
			txtField.addEventFilter(KeyEvent.KEY_PRESSED, (e) -> {								
				...
				if (Character.isLowerCase(e.getCode().getChar().charAt(0))) {
					// En principio la validación con KEY_PRESSED no funciona porque se lanza igualmente KEY_TYPED
					e.consume();
				}
			});
